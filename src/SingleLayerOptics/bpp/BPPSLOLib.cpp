...
private:
  ...
  //Window stuff
  std::shared_ptr<CSeries> radiationSpectra;
  std::shared_ptr<CSpectralSampleData> spectralSampleData;
  //Material material;
  //CBSDFHemisphere BSDF;
  std::shared_ptr<CBSDFLayer> layer;
  //The results
  std::shared_ptr<CBSDFIntegrator> results;
public:
  //The constructor
  SLOSystem(){
    this->set = false;
    this->solved = false;
    this->materialType = "Monolithic";//default value
    this->wavelengthRange = "Solar";//default value
  }//SLOSystem(){...}
  //----------------------------------------------------------------------------
  //Initialize
  void initialize(){
    //check the material type
    auto matType = MaterialType::Monolithic;
    if (this->materialType.compare("Monolithic") == 0  ){
      matType = MaterialType::Monolithic;
    } else if (this->materialType.compare("Coated") == 0 ){
      matType = MaterialType::Coated;
    } else if (this->materialType.compare("Film") == 0){
      matType = MaterialType::Film;
    } else if (this->materialType.compare("AppliedFilm") == 0){
      matType = MaterialType::AppliedFilm;
    } else if (this->materialType.compare("Laminate") == 0){
      matType = MaterialType::Laminate;
    } else if (this->materialType.compare("Interlayer") == 0){
      matType = MaterialType::Interlayer;
    } else if (this->materialType.compare("Electrochromic") == 0){
      matType = MaterialType::Electrochromic;
    } else if (this->materialType.compare("Thermochromic") == 0){
      matType = MaterialType::Thermochromic;
    }
    //check the wavelength range
    auto range = WavelengthRange::Solar;
    if (this->wavelengthRange.compare("Solar") == 0  ){
      range = WavelengthRange::Solar;
    } else if (this->wavelengthRange.compare("IR") == 0  ){
      range = WavelengthRange::IR;
    } else if (this->wavelengthRange.compare("Visible") == 0  ){
      range = WavelengthRange::Visible;
    }
    //create Material
    auto aMaterial = Material::nBandMaterial(
      this->spectralSampleData, this->thickness, matType, range);
    //create a BSDF
    const auto aBSDF = CBSDFHemisphere::create(BSDFBasis::Full);
    //this->BSDF = CBSDFHemisphere::create(BSDFBasis::Full);
    //create a Layer
    layer = CBSDFLayerMaker::getSpecularLayer(aMaterial,aBSDF);
    layer->setSourceData(this->radiationSpectra);
    //create a layer
    this->set=true;
  }//void initialize(){...}
  //----------------------------------------------------------------------------
  boost::python::dict getBasicResults(){
    boost::python::dict out;
    out["TDirDir"] = this->results->DirDir(Side::Front, PropertySimple::T, 0, 0);
    out["RfDirDir"] = this->results->DirDir(Side::Front, PropertySimple::R, 0, 0);
    out["RbDirDir"] = this->results->DirDir(Side::Back, PropertySimple::R, 0, 0);
    out["TDiffDiff"] = this->results->DiffDiff(Side::Front, PropertySimple::T);
    out["RfDiffDiff"] = this->results->DiffDiff(Side::Front, PropertySimple::R);
    out["RbDiffDiff"] = this->results->DiffDiff(Side::Back, PropertySimple::R);
    return out;
  }//getBasicResults(){...}
  //----------------------------------------------------------------------------
  void setDefaultRadiationSpectra(){
    // Full ASTM E891-87 Table 1 (Solar radiation)
    this->radiationSpectra = CSeries::create({
 ...
 //-----------------------------------------------------------------------------
 void setMaterialType(std::string matType){
   this->materialType=matType;
 }//void setMaterialType(std::string matType){
 //-----------------------------------------------------------------------------
 void setRadiationSpectra(boost::python::list& wavelengths, boost::python::list& values){
   this->radiationSpectra = CSeries::create();
   for (int i=0; i<len(wavelengths); i++){
     this->radiationSpectra->addProperty(boost::python::extract<double>(wavelengths[i]),boost::python::extract<double>(values[i]));
   }
 }//void setRadiationSpectra(){...}
 //----------------------------------------------------------------------------
 void setSpectralSampleData(boost::python::list& wavelengths,boost::python::list& T,boost::python::list& Rf,boost::python::list& Rb){
   this->spectralSampleData = CSpectralSampleData::create();
   for (int i=0; i<len(wavelengths); i++){
     this->spectralSampleData->addRecord(boost::python::extract<double>(wavelengths[i]),
      boost::python::extract<double>(T[i]),
      boost::python::extract<double>(Rf[i]),
      boost::python::extract<double>(Rb[i])
     );
   }//for (int i=0; i<len(wavelengths); i++){
 }//void setSpectralSampleData(){...}
 //-----------------------------------------------------------------------------
 void setTestSpectralSampleData(){
   this->spectralSampleData = CSpectralSampleData::create(
 ...
 //-----------------------------------------------------------------------------
 void setWavelengthRange(std::string range){
   this->wavelengthRange=range;
 }//void setWavelengthRange(std::string range){
 //-----------------------------------------------------------------------------
 void solve(){
   if (this->set){
     this->results = this->layer->getResults();
     this->solved = true;
   }
 }//void solve(){...}
 //-----------------------------------------------------------------------------
...
BOOST_PYTHON_MODULE(BPPSLOLib)
{
  using namespace boost::python;
  class_<SLOSystem>("SLOSystem", init<>())
    .def("initialize", &SLOSystem::initialize)
    .def("getBasicResults", &SLOSystem::getBasicResults)
    .def("solve", &SLOSystem::solve)
    .def("setDefaultRadiationSpectra", &SLOSystem::setDefaultRadiationSpectra)
    .def("setMaterialType", &SLOSystem::setMaterialType)
    .def("setSpectralSampleData", &SLOSystem::setSpectralSampleData)
    .def("setRadiationSpectra", &SLOSystem::setRadiationSpectra)
    .def("setTestSpectralSampleData", &SLOSystem::setTestSpectralSampleData)
    .def("setWavelengthRange", &SLOSystem::setWavelengthRange)
    .def("setThickness", &SLOSystem::setThickness);
}
